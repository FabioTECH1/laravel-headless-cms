    ...
    <div class="h-screen w-full bg-gray-50 dark:bg-gray-900 flex">
        ...
        <aside class="w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col">
            ...
            <nav class="flex-1 px-4 py-4 space-y-1">
                ...
                <Link v-if="isSuperAdmin()" href="/admin/roles"
                    class="flex items-center px-2 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 group"
                    :class="{ 'bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white': $page.url.startsWith('/admin/roles') }">
                    Roles & Permissions
                </Link>

                <!-- Webhooks (Super Admin only) -->
                <Link v-if="isSuperAdmin()" href="/admin/webhooks"
                    class="flex items-center px-2 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 group"
                    :class="{ 'bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white': $page.url.startsWith('/admin/webhooks') }">
                    Webhooks
                </Link>
